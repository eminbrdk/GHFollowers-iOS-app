...
class GFUserInfoHeaderVC: UIViewController {
    ...
    let userNameLabel = GFTitleLabel(textAlign: .left, fontSize: 34, text: "aaa")
    let nameLabel = GFSecondaryTitleLabel(fontSize: 18)
    let locationImageView = UIImageView()
    let locationLabel = GFSecondaryTitleLabel(fontSize: 18)
    let bioLabel = GFBodyLabel(textAlignment: .left, text: "aşdklsl")
    ...
    func configureUIElements() {
        ...
        nameLabel.text = user.name ?? "" // user.name optional olduğu için nil olursa boş string gelsin
